...
def aedilatedfusionnetwork_model(input_shape=(1, 128, 128, 128), n_base_filters=16, depth=5, dropout_rate=0.3,
                      n_segmentation_levels=3, n_labels=4, optimizer=Adam, initial_learning_rate=5e-4,
                      loss_function=weighted_dice_coefficient_loss, activation_name="softmax", multi_gpu_flag=False):
    ...
    for level_number in range(depth):
        ...
        if current_layer is inputs:
            in_conv = create_inception_module(current_layer, n_level_filters, concat=True)
...
def create_inception_module(input_layer, n_filters, strides=(1,1,1), concat=True, upsample=False):
    in_conv_1 = create_convolution_block(input_layer, n_filters, kernel=(3,3,3), strides=(1,1,1), instance_normalization=True, batch_normalization=False, dilation_rate=1)
    in_conv_2 = create_convolution_block(input_layer, n_filters, kernel=(3,3,3), strides=(1,1,1), instance_normalization=True, batch_normalization=False, dilation_rate=2)
    in_conv_3 = create_convolution_block(input_layer, n_filters, kernel=(3,3,3), strides=(1,1,1), instance_normalization=True, batch_normalization=False, dilation_rate=3)
    ...
    in_conv = create_convolution_block(in_concat, n_filters, kernel=(1,1,1), instance_normalization=True, dilation_rate=1)
    if strides == (2,2,2):
        mpool_1 = MaxPooling3D((2, 2, 2), padding='valid', strides=(2,2,2)) (in_conv)
    else:
        mpool_1 = in_conv
    return mpool_1