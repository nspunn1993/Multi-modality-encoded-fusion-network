...
import numpy as np
import tables
from .model import aefusionnetwork_model
...
def run_validation_case(data_index, output_dir, model, data_file, training_modalities,
    ...
    test_data = np.asarray([data_file.root.data[data_index],data_file.root.data[data_index]])
    ...
    if patch_shape == test_data[0].shape[-3:]:
...
def run_validation_cases(validation_keys_file, model_file, training_modalities, labels, hdf5_file,
    ...
    print("Run Validation Cases")
    validation_indices = pickle_load(validation_keys_file)
    print("Loading Old Model")
    '''
    model = aefusionnetwork_model(input_shape=(1,144,144,144), n_labels=3,
                                  initial_learning_rate=0.00001,
                                  n_base_filters=8, multi_gpu_flag=True)
...
def predict(model, data, permute=False):
    if permute:
        ...
        print("Datashape end")
        return model.predict([data[:,0:1,:,:,:],data[:,1:2,:,:,:],data[:,2:3,:,:,:],data[:,3:4,:,:,:]])


def predict_with_permutations(model, data):
    predictions = list()
    print("predict_with_permutations")
    for permutation_key in generate_permutation_keys():
        temp_data3 = permute_data(data, permutation_key)[np.newaxis]
        print(temp_data3.s)
        temp_data = [temp_data3[:,0:1,:,:,:],temp_data3[:,1:2,:,:,:],temp_data3[:,2:3,:,:,:],temp_data3[:,3:4,:,:,:]]
